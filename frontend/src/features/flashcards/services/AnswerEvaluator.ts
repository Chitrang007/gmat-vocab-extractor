...
export class AnswerEvaluator {
  evaluate(input: string, round: Round): EvaluationResult {
    ...
    for (const expected of normalizedExpectedList) {
      bestConfidence = Math.max(bestConfidence, similarityScore(normalizedInput, expected))
    }

    const isCorrect = bestConfidence >= FUZZY_THRESHOLD
    const matchType: MatchType = isCorrect ? 'fuzzy' : bestConfidence >= FUZZY_PARTIAL_THRESHOLD ? 'fuzzy' : 'none'
